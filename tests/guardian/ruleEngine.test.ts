...
function debugLog(scenarioName: string, event: any, alert: any) {
  if (process.env.DEBUG) {
    console.log(`[${scenarioName}] ${event.type} (${event.id}) -> Alert:`, 
...
describe('Rule Engine Scenarios', () => {
  ...
  it.each(scenarios)('scenario %s emits the expected alerts', (scenarioName) => {
    ...
    scenarioEvents.forEach((event, index) => {
      ...
      if (scenarioName === 'velocity-breach' && event.type === 'payout.paid') {
        ...
        if (history.length > 0) {
          console.log(`  Recent payout history: ${history.filter(e => e.type === 'payout.paid').length} events`);
          history.filter(e => e.type === 'payout.paid').forEach(e => {
    ...
    if (emittedAlerts.length > 0) {
      ...
      emittedAlerts.forEach((alert, i) => {
        console.log(`  [${i}] type=${alert.type}, severity=${alert.severity}, id=${alert.payoutId || alert.externalAccountId}`);
      });
    }
  ...
  it('does not leak state between test runs', () => {
    ...
    const velocityEvents = loadScenario('velocity-breach')
      .map(scenarioEventToGuardianEvent);
