...
describe('Stripe OAuth Flow', () => {
  ...
  describe('OAuth Callback Route', () => {
    it('should return 400 if state does not match', async () => {
      ...
      const mockRequest = new Request('https://test.example.com/api/stripe/oauth/callback?code=test_code&state=different-state');
    ...
    it('should store tokens in database when OAuth flow is successful', async () => {
      ...
      const mockRequest = new Request(`https://test.example.com/api/stripe/oauth/callback?code=${mockCode}&state=${mockState}`);
