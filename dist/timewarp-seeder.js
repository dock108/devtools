...
// Determine base path for scenarios, handle both Vercel deployment and local dev
...
export async function runSeeder() {
    ...
    try {
        ...
        if (actionRoll < 0.6 && currentBalance > 0) {
            ...
            try {
                if (SCENARIOS.length > 0) {
                    ...
                    const events = JSON.parse(content).map((event) => ({
                        ...event,
        ...
        console.log(`[INFO] Time-warp tick complete for ${acct}: charge=$${chargeAmt / 100}${payoutAmt !== undefined ? `, payout=$${payoutAmt / 100}` : ''}, balance=${finalBalance === -1 ? 'Fetch Error' : `$${finalBalance / 100}`}`);
        return {
            account: acct,
