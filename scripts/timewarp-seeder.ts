...
async function runSeedCycle() {
  ...
  const ACCOUNTS = ACCOUNTS_RAW.split(',').map(acc => acc.trim()).filter(Boolean);
  ...
  try {
    ...
    if (currentBalance < 10000) {
      ...
      try {
        ...
        logger.error({ acct, amount: initialFundingAmount / 100, error }, 'Failed to add initial funding');
    ...
    } else if (actionRoll >= 0.6) { // Using >= 0.6 for the remaining 40%
      // 40% chance: Inject Scenario (Simplified: just log for now)
      try {
        ...
        if (jsonFiles.length > 0) {
          ...
          // TODO: Implement actual injection if needed. For now, just log.
          // Reading the file to ensure it's accessible as per AC.
          await fs.readFile(fixturePath); 
          logger.info({ acct, fixture: randomFixture, speedFactor: SPEED_FACTOR }, 'Selected fixture for injection (logging only)');
          // Example of modification (not used yet):
          // const content = await fs.readFile(fixturePath, 'utf-8');
          // const scenario = JSON.parse(content);
          // scenario.events.forEach(event => event.delayMs = Math.max(0, event.delayMs / SPEED_FACTOR));
        } else {
          logger.warn({ directory: FIXTURES_DIR }, 'No scenario fixtures found for injection');
        }
      } catch (error) {
        logger.error({ acct, directory: FIXTURES_DIR, error }, 'Failed to read or process scenario fixtures');
      }
    }
