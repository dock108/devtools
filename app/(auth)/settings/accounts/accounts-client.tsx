...
export default function AccountsClient({ initialAccounts }: { initialAccounts: Account[] }) {
  ...
  const disconnectAccount = async (accountId: string) => {
    if (
      !confirm('Are you sure you want to disconnect this account? This action cannot be undone.')
    ) {
      return;
    }
      ...
      {accounts.map((account) => (
        <Card key={account.stripe_account_id} className="overflow-hidden">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription className="text-xs mt-1">
                  {account.stripe_account_id} · Connected{' '}
            ...
            <div className="flex flex-col gap-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <Label
                    htmlFor={`auto-pause-${account.stripe_account_id}`}
              ...
              <p className="text-sm text-slate-500">
                When enabled, payouts will be automatically paused when high-severity fraud alerts
                are detected.
              </p>
            </div>
          </CardContent>
          ...
          <CardFooter className="flex justify-between border-t bg-slate-50 px-6 py-4">
            ...
            >
              <RefreshCcw
                className={`h-4 w-4 mr-2 ${loadingStates[`rotate_${account.stripe_account_id}`] ? 'animate-spin' : ''}`}
              />
              {loadingStates[`rotate_${account.stripe_account_id}`]
                ? 'Rotating...'
                : 'Rotate Webhook Secret'}
            </Button>
            ...
            >
              ...
              {loadingStates[`disconnect_${account.stripe_account_id}`]
                ? 'Disconnecting...'
                : 'Disconnect Account'}
            </Button>
          </CardFooter>
