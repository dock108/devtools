...
export function ConnectedAccountsManager({ initialAccounts }: { initialAccounts: ConnectedAccount[] }) {
  ...
  const handleToggleMute = (account: ConnectedAccount) => {
    ...
    startMuteToggleTransition(async () => {
      const isCurrentlyMuted = !!account.alerts_muted_until && new Date(account.alerts_muted_until) > new Date();
      ...
      const optimisticMutedUntil = action === 'mute' 
        ? (durationMinutes === Infinity ? 'infinity' : new Date(Date.now() + durationMinutes * 60000).toISOString())
        : null;
  ...
  const getMuteStatus = (account: ConnectedAccount): { muted: boolean; text: string } => {
    if (account.alerts_muted_until) {
        ...
        if (muteEndDate > new Date()) {
            return { muted: true, text: `Muted ${formatDistanceToNow(muteEndDate, { addSuffix: true })}` };
        }
    }
  ...
  return (
    <Card>
      ...
      <CardContent>
        {accounts.length === 0 ? (
          <p className="text-center text-muted-foreground py-8">No Stripe accounts connected yet.</p>
        ) : (
              ...
              <Table>
                ...
                <TableBody>
                  {accounts.map((account) => {
                    ...
                    const isLoadingMute = isTogglingMute && togglingMuteAccountId === account.stripe_account_id;
                    return (
                      <TableRow key={account.id}>
                        ...
                        <TableCell className="flex items-center gap-2">
                          ...
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => copyToClipboard(account.stripe_account_id)}><Copy className="h-3 w-3" /></Button>
                        </TableCell>
                        <TableCell>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div className="flex items-center">
                                <Switch
                                  ...
                                  disabled={isTogglingPayouts && togglingPayoutsAccountId === account.stripe_account_id}
                                  aria-label={account.payouts_paused ? 'Resume payouts' : 'Pause payouts'}
                                />
                                {isTogglingPayouts && togglingPayoutsAccountId === account.stripe_account_id && <Loader2 className="h-4 w-4 animate-spin ml-2" />}
                              </div>
                            </TooltipTrigger>
                          ...
                          <div className="flex items-center space-x-2">
                            <Tooltip>
                              <TooltipTrigger asChild>
                                ...
                                >
                                  {isLoadingMute ? <Loader2 className="h-4 w-4 animate-spin" /> : <BellOff className={`h-4 w-4 ${muteStatus.muted ? 'text-destructive' : 'text-muted-foreground'}`} />}
                                </Button>
                              </TooltipTrigger>
                              <TooltipContent>
                                {muteStatus.muted ? 'Click to unmute alerts' : 'Click to mute alerts'}
                                <br />
                                <span className="text-xs text-muted-foreground">Monitoring continues while muted.</span>
                              </TooltipContent>
                            </Tooltip>
                            {muteStatus.muted ? (
                              ...
                              <Select value={muteDuration} onValueChange={setMuteDuration} disabled={isLoadingMute}>
                        ...
                        <TableCell className="text-right">
                          <AlertDialog open={accountToDisconnect === account.stripe_account_id} onOpenChange={(open) => !open && setAccountToDisconnect(null)}>
                            <AlertDialogTrigger asChild>
                              <Button
                                ...
                                disabled={isDisconnecting && accountToDisconnect === account.stripe_account_id}
                              >
                                {isDisconnecting && accountToDisconnect === account.stripe_account_id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                              </Button>
                            </AlertDialogTrigger>
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  This will disconnect the Stripe account <span className="font-medium">{account.business_name || account.stripe_account_id}</span> from DOCK108. This action cannot be undone.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                                <AlertDialogCancel onClick={() => setAccountToDisconnect(null)}>Cancel</AlertDialogCancel>
                                <AlertDialogAction
                    ...
                    <CardContent className="space-y-3 text-sm">
                      <div className="flex items-center gap-2 text-muted-foreground">
                         <span className="font-mono">{account.stripe_account_id}</span>
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => copyToClipboard(account.stripe_account_id)}>
                            <Copy className="h-3 w-3" />
                          </Button>
                      </div>
                      <p className="text-muted-foreground">Connected: {format(new Date(account.created_at), 'MMM d, yyyy')}</p>
                      <div className="flex items-center justify-between pt-2 border-t">
                        ...
                        <Tooltip>
                           ...
                           <TooltipContent>
                             {getPauseTooltipContent(account)}
                           </TooltipContent>
                         </Tooltip>
                      </div>
                      <div className="flex items-center justify-between pt-2 border-t">
