...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (error) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to send digest email', details: error.message }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(JSON.stringify({ success: true, recipients: uniqueNewEmails.length, messageId: data?.id }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
